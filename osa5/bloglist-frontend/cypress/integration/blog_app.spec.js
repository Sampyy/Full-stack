describe('Blog app', function() {
    beforeEach(function() {
        ...
        cy.request('POST', 'http://localhost:3003/api/users', {
            username: 'testingUser', name: 'testingName' ,password: 'secretpw'
        })
    ...
    describe('When logged in', function() {
        ...
        it('A blog can be created', function() {
            ...
            cy.get('.success')
                .should('contain', 'Added a new blog: testingTitle by testingAuthor')
        ...
        describe('and it exists', function() {
            beforeEach(function() {
                cy.createBlog({ author: 'testingAuthor', title: 'testingTitle', url: 'testingUrl' })
            })
            it('it can be liked', function() {
                cy.contains('testingTitle testingAuthor').contains('show').click()
                cy.get('html').should('not.contain', 'likes 1')
                cy.contains('testingTitle').parent().contains('like').find('button').click()
            ...
            it('it can be deleted by the correct user', function() {
                ...
                cy.get('html').should('not.contain', 'testingTitle testingAuthor')
            })
            it('and it cannot be deleted by a different user', function() {
                cy.contains('Log out').click()
                cy.request('POST', 'http://localhost:3003/api/users', {
                    username: 'userNotGood', name: 'badUser' ,password: 'secretpw'
                })
                cy.login({ username: 'userNotGood', password:'secretpw' })
                cy.contains('testingTitle testingAuthor').contains('show').click()
                cy.contains('testingTitle').parent()
        ...
        describe('multiple blogs are in correct order', function() {
            ...
            it('and when liking one, it will be first', function() {
                ...
                cy.contains('title4').parent().contains('show').click()
            ...
            it('and when liking multiple, they will be in correct order', function() {
                ...
                cy.contains('title2').parent().contains('show').click()
                cy.contains('title3').parent().contains('show').click()
                cy.contains('title4').parent().contains('show').click()
                cy.contains('title4').parent().contains('Like').click()
